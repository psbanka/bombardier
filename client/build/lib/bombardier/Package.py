                ...
                if self.metaData.data.get("install"):
                    if type(self.metaData.data["install"]) == type({}):
                        ...
                        if self.fullName:
                            ...
                            self.logger.warning("No full name defined in metadata")
                    else:
                        self.logger.warning("'install' section is not a dictionary")
                else:
                    self.logger.warning("No 'install' section in metadata")
            else:
                self.logger.warning("Metadata is not a dictionary")
        else:
            self.logger.warning("No metadata found for this package")
        return FAIL

    def initialize(self):
        self.filesystem.updateCurrentAction("Initializing package.", 0)
        self.metaData = self.repository.getMetaData(self.name)
        if self.checkMetaData() == OK:
